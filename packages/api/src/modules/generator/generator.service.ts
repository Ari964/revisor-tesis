...
export class GeneratorService {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.apiModel = this.configService.get<string>('GEMINI_MODEL') || 'gemini-2.5-flash-lite';
  }
  ...
  private async callGemini(systemPrompt: string, userPrompt: string, isJson = true): Promise<any> {
    ...
    try {
      ...
      throw new InternalServerErrorException(`Fallo en el servicio de IA: ${error.message}`);
    }
  }
