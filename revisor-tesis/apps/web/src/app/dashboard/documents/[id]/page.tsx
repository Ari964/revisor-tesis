...
export default function DocumentDetailPage() {
  ...
  const handleFeedback = async (findingId: string, action: 'ACCEPTED' | 'MODIFIED' | 'DISCARDED', newText?: string) => {
    // In a real app this would POST to /api/review/feedback
    console.log('Feedback submitted:', { findingId, action, newText });
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="glass-card p-6 min-h-[400px] animate-slide-up">
        ...
        {activeTab === 'citations' && (
          <div className="space-y-4">
            <h2 className="text-xl font-semibold flex items-center gap-2"><Quote className="w-5 h-5 text-blue-500" /> Validación de Citas (CrossRef)</h2>
            {doc.citationValidations && doc.citationValidations.length > 0 ? (
              <div className="space-y-3 mt-4">
                {doc.citationValidations.map((validation: any, idx: number) => (
                  <div key={idx} className={`p-4 rounded-xl border ${validation.status === 'VERIFIED' ? 'bg-emerald-50 dark:bg-emerald-900/10 border-emerald-100 dark:border-emerald-900/30' : validation.status === 'NOT_FOUND' ? 'bg-red-50 dark:bg-red-900/10 border-red-100 dark:border-red-900/30' : 'bg-yellow-50 dark:bg-yellow-900/10 border-yellow-100 dark:border-yellow-900/30'}`}>
                    <div className="flex justify-between items-start mb-2">
                      <span className={`font-semibold ${validation.status === 'VERIFIED' ? 'text-emerald-700 dark:text-emerald-400' : validation.status === 'NOT_FOUND' ? 'text-red-700 dark:text-red-400' : 'text-yellow-700 dark:text-yellow-400'}`}>
                        {validation.status === 'VERIFIED' ? 'Cita verificada' : validation.status === 'NOT_FOUND' ? 'Cita no encontrada' : 'Coincidencia parcial'}
                      </span>
                      {validation.matchScore && (
                        <span className="text-xs font-medium px-2 py-1 bg-blue-200 dark:bg-blue-800 text-blue-800 dark:text-blue-200 rounded-full">
                          Score: {Math.round(validation.matchScore * 100)}%
                        </span>
                      )}
                    </div>
                    <p className="text-sm mt-2 italic">"{validation.rawCitation}"</p>
                    {validation.crossrefTitle && (
                      <div className="mt-2 text-xs">
                        <span className="font-medium">Título CrossRef:</span> {validation.crossrefTitle}
                      </div>
                    )}
                    {validation.crossrefDoi && (
                      <div className="mt-1 text-xs">
                        <span className="font-medium">DOI:</span> {validation.crossrefDoi}
                      </div>
                    )}
                  </div>
                ))}
              ...
              <p className="text-gray-500">No se han validado citas o el análisis está en curso.</p>
            )}
          </div>
        )}
      </div>
